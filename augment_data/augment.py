...
def get_phobert_model():
    model_dir = 'PhoBERT_base_fairseq'
    checkpoint_file = 'model.pt'
    bpe_codes_file = 'bpe.codes'

    # Full paths for model and BPE codes
    model_path = os.path.join(model_dir, checkpoint_file)
    bpe_codes_path = os.path.join(model_dir, bpe_codes_file)

    # Check if files exist
    if not os.path.exists(model_path) or not os.path.exists(bpe_codes_path):
        raise FileNotFoundError(f"Required files not found in {model_dir}. Ensure both {checkpoint_file} and {bpe_codes_file} are present.")

    # Load the PhoBERT model
    phoBERT = RobertaModel.from_pretrained(model_dir, checkpoint_file=checkpoint_file)
    phoBERT.to("cuda" if torch.cuda.is_available() else "cpu")
    phoBERT.eval()
...
def read_data(path):
    with open(path, 'r', encoding='utf-8') as file:
        data = []
        for line in file:
            # Assuming class labels are digits followed by a space and then text
            parts = line.split(maxsplit=1)
            if len(parts) == 2:
                class_label, text_data = parts
                class_label = class_label  # Convert class label to integer if appropriate
                data.append([class_label, text_data.strip()])
            else:
                print(f"Skipped malformed line: {line}")

        # Create a DataFrame
        df = pd.DataFrame(data, columns=['Class', 'Data'])
        return df
...
def main(args):
    ...
    for i, sentence in tqdm(enumerate(data['Data'])):
        result.append([data['Class'][i], sentence])
        if data['Class'][i] in none_augment:
            continue
